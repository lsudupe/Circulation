...
enrichment.meta <- read.csv( "./data/infarto_5dpi_hembra/enrichment_2.csv")
...
pdf(file.path("./results/individual/2.prueba",filename = paste("relation_log.d1.d2.boxplot.pdf",sep="")))
dpi5_female@meta.data%>% 
  ggplot(aes(x=relation_log.d1.d2, y= segmen, fill=segmen)) + 
  geom_boxplot() +  
  theme_classic() +
